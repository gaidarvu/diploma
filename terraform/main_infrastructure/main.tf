...
data "template_file" "cloudinit-master" {
  template                = file("${path.module}/cloud-init/cloud-init-master.yaml")
  vars = {
    vms_ssh_root_key      = var.vms_ssh_root_key
    ssh_private_key       = var.master_ssh_private_key
    inventory_content     = local.inventory_content
  }
}

data "template_file" "cloudinit-worker" {
  template                = file("${path.module}/cloud-init/cloud-init-worker.yaml")
  vars = {
    vms_ssh_root_key      = var.vms_ssh_root_key
  }
}
...
resource "yandex_compute_instance" "worker_instances" {
  ...
  scheduling_policy {
    ...
    metadata = {
      user-data           = data.template_file.cloudinit-worker.rendered
